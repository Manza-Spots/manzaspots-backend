...
class SentryErrorHandlerMixin:
    ...
    def handle_with_sentry(
        ...
        Args:
            ...
            audit_action: Nombre de la acción para auditoría
        ...
        # Log de inicio (auditoría)
        if audit_action and self.log_successful_operations:
            self._log_audit_start(audit_action, request, extra)
        ...
        try:
            ...
            # Log de éxito
            if audit_action and self.log_successful_operations:
                self._log_audit_success(audit_action, extra)
    ...
    def _log_audit_start(self, action, request, extra):
    ...
    def _log_audit_success(self, action, extra):
        """Log de éxito de operación"""
        audit_logger.info(
            f"Completado: {action}",
            extra={'action': f"{action}_success", **extra}
        )